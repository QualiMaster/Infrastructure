...
/** 
 * The data output that buffers data from the replay recorder
 * and puts them into the underlying replay store
 *
 * @author tuan
 * @version 1.0
 */
...
public class ReplayDataOutput implements IDataOutput, Closeable {
    ...
    /**
     * Field are used to match to the column (or in Hbase,
    ...
    private void writeIfNeeded() throws UnsupportedEncodingException {
        ...
        if (idx == fields.length) {
            ...
            /* In HBase, we append row key with the time step
	...
	public void writeStringArray(String[] array) throws IOException {
        ...
        // TODO: For now, string array cannot be timestamp field (might reconsider this !)
        else {
            row.addValue(bytes);
        }
        writeIfNeeded();
