...
public class DataManager {
    ...
    public static class DataSinkManager extends AbstractDataManager<IDataSink> {
        ...
        public <S extends IDataSink> S createDataSink(String container, Class<S> cls,
            ...
            initialize(); // ensure event handler registration on workers
    ...
    static {
        if (!isInitialized) {
            // force client initialization so that remote handlers can be registered, already initialized for server
            // event will be ignored
            EventManager.send(new AbstractEvent() {
                private static final long serialVersionUID = 8290855329405636058L;
            });
            // register 
