...
public class DataManager {
    ...
    public static final AbstractStorageManager<?> INTERMEDIARY_STORAGE_MANAGER 
        = new HBaseStorageManager("intermediary.");
    ...
    /**
     * The temporary data manager for replay mechanism - Default Redis-based key-value store
     */
    ...
    private static class PipelineLifecycleEventEventHandler extends EventHandler<PipelineLifecycleEvent> {
        ...
        protected void handle(PipelineLifecycleEvent event) {
            ...
            case INITIALIZED:
                ...
                // TODO this is just a workaround and shall be removed as soon as possible
    ...
    private static class ReferenceDataManagementEventHandler extends EventHandler<ReferenceDataManagementEvent> {
        ...
        protected void handle(ReferenceDataManagementEvent event) {
            ...
            if (localMode) {
                // don't handle register/dispose as these shall be local references
                relevant = Command.CONNECT == cmd || Command.DISCONNECT == cmd;
            } else {
                if (started) { // we are the central instance, handle only register/dispose
                    relevant = Command.REGISTER == cmd || Command.DISPOSE == cmd;
                } else { // we are on a Storm node, handle connect and disconnect sent by central instance
                    relevant = Command.CONNECT == cmd || Command.DISCONNECT == cmd;
                }
            }
            if (relevant) {
    ...
    public static class DataSinkManager extends AbstractDataManager<IDataSink> {
        ...
        /**
         * Creates a data sink for <code>cls</code> and assigns it to <code>container</code>. Please note that multiple
         ...
         * @param container the (symbolic) name of a container to assign the created instance to,
         *   no assignment will happen if <code>container</code> is <b>null</b>, but then also 
         *   {@link #connectAll(String)},
         *   {@link #disconnectAll(String)} and {@link #discardAll(String)} will be useless.
         * @param cls the class of the data sink to be created
         * @param strategy the desired data storage strategy
    ...
    public static class DataSourceManager extends AbstractDataManager<IDataSource> {
        ...
        /**
         * Creates a data source for <code>cls</code> and assigns it to <code>container</code>. Please note that 
         * multiple different data sources for one <code>cls</code> may be created, but the underlying factory may 
         * also just return individual proxies to the same physical data source (e.g., hidden behind a 
         * buffering mechanism).
         * 
         * @param <S> the actual type to be created
         * @param container the (symbolic) name of a container to assign the created instance to,
         ...
         *   {@link #connectAll(String)},
         *   {@link #disconnectAll(String)} and {@link #discardAll(String)} will be useless.
         * @param cls the class of the data source to be created
         * @param strategy the desired data storage strategy
         * @return an instance of <code>cls</code> in the most easiest case, a modified/wrapped 
         *   instance of <code>cls</code> in case of transparent data storage, <b>null</b> if no instance
         *   can be created, e.g., in case that <code>cls</code> is an abstract class or an interface
    ...
    public static class HBaseStorageManager extends eu.qualimaster.dataManagement.storage.hbase.HBaseStorageManager {
         ...
         * Prevents external creation.
         * 
         * @param tablePrefix a fixed prefix internally prepended before each table name
         */
    ...
    /**
     * Customize HBaseStorageManager and override Patrick's HBaseStorageSupport implementation in order to:
     * - Allow bulk put into the table
     * - Be able to extend to other efficient Time series DB like OpenTSDB in the future
     * 
     * @author Tuan
     */
     ...
     * method only on the central platform. For tests, see {@link #start(boolean)}. 
     */
    ...
    /**
     * Start the data management layer. Please call this method only on the central platform (with 
     * <code>testMode</code> set to <code>false</code> / via {@link #start()}) or, for tests, 
     * in the JVM of the local cluster (with <code>testMode</code> set to <code>true</code>).
     * 
     * @param testMode run the data management layer in test mode
     */
    ...
    /**
     * Called as part of the shutdown process of the QualiMaster platform. Please call this 
     * method only on the central platform or, for tests, in the JVM of the local cluster.
     */
    ...
    /**
     * Determines the sequence of managers for startup (connect) or shutdown (disconnect). Actually, this works by
     * partitioning all known managers in source, sink and other managers and recombining the sequence accordingly.
     * 
     * @param connect whether the connect or disconnect sequence shall be determined
    ...
    private static List<AbstractDataManager<?>> determineSequence(boolean connect) {
        ...
        if (connect) {
            ...
            // on disconnect, sources shall be first to avoid additional incoming data
            result.addAll(sourceManagers);
            result.addAll(sinkManagers);
            result.addAll(otherManagers);
        }
     ...
     * 
     * @param unit the container (no operation if <b>null</b>) 
     */
