 ...
 * historical values and unaware of recent values
 * 
...
public class BlindPrediction {
    ...
    /**
     * The set of average volume values, at each minute of the day, used to
     * estimate the volume of a source without knowing its recent values.
     */
    ...
    /**
     * Estimates the volume at the current time (minute) of the day by computing
     * the average over the volumes at the same time of the day in different
     * past days. This can be used without knowing the sequence of recent
     * volumes (e.g. to estimate the volume introduced by crawling a stock
     * before actually doing it).
     * 
     * @return The estimated volume at the current time of the day (-1 if no
    ...
    public double predictBlindly() {
        ...
        if (this.historicalVolumes == null || this.historicalVolumes.isEmpty()) {
            System.out
                    .println("Not historical data available to make predictions.");
            return -1;
        }
        ...
        if (this.getHistoricalVolumes().containsKey(time))
            return this.getHistoricalVolumes().get(time);
        else
            return getNeighborValue(time);
    }
    ...
    private HashMap<String, Double> computeAverageVolumes(
        ...
        for (String k : map.keySet()) {
            ...
            if (mapByTime.containsKey(time))
                mapByTime.get(time).add(map.get(k));
            else {
    ...
    private double getNeighborValue(String time) {
        ...
        while (neighborValue == -1) {
            ...
            if (prevHour < 10)
                ...
                prevTime += prevHour + ":";
            if (prevMinute < 10)
                prevTime += "0" + prevMinute + ":";
            else
                prevTime += prevMinute + ":";
            if (prevSecond < 10)
                prevTime += "0" + prevSecond;
            else
                prevTime += prevSecond;
            ...
            if (succHour < 10)
                ...
                succTime += succHour + ":";
            if (succMinute < 10)
                succTime += "0" + succMinute + ":";
            else
                succTime += succMinute + ":";
            if (succSecond < 10)
                succTime += "0" + succSecond;
            else
                succTime += succSecond;

            if (this.historicalVolumes.containsKey(prevTime))
                neighborValue = this.historicalVolumes.get(prevTime);
            if (this.historicalVolumes.containsKey(succTime))
                neighborValue = this.historicalVolumes.get(succTime);
        }
