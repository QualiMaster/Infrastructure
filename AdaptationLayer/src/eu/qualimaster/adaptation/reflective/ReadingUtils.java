...
public class ReadingUtils {
	...
	private Platform readPlatform(String[] fields) throws ParseException{
		...
		while(fields[++this.counter].compareTo(PIPELINE_TAG) != 0){
			if(fields[this.counter].compareTo("") != 0 && fields[this.counter].compareTo(" ") != 0){
				double value = this.numberFormat.parse(fields[this.counter]).doubleValue();
				measures.add(value);
			}
			else{
				measures.add(-1.0);
	...
	private Pipeline readPipeline(String[] fields) throws ParseException{
		...
		while(this.counter < fields.length - 1 && fields[this.counter].compareTo(PIPELINE_TAG) != 0){
			this.counter++;
	...
	private Node readNode(String[] fields) throws ParseException{
		...
		// set node measures
		ArrayList<Double> measures = new ArrayList<>();
		while(++this.counter < fields.length-1 && fields[this.counter].compareTo(NODE_TAG) != 0 && fields[this.counter].compareTo(PIPELINE_TAG) != 0){
			if(fields[this.counter].compareTo("") != 0 && fields[this.counter].compareTo(" ") != 0){
				double value = this.numberFormat.parse(fields[this.counter]).doubleValue();
				measures.add(value);
			}
