...
public class AdaptationLoggerFile implements IAdaptationLogger {
	...
	/** The current adaptation, filled in the methods between startAdaptation() and endAdaptation(), which are sequential */
	private AdaptationLoggerUnit currentAdaptation;
	
	/** The column headers*/
	private static final String[] COLUMN_NAMES = {"START_TIME", "END_TIME", "EVENT", "CONDITION", "STRATEGY", "STRATEGY_SUCCESS",
												  "TACTIC", "TACTIC_SUCCESS", "MESSAGE", "ADAPTATION_SUCCESS"};
    ...
    public void startAdaptation(AdaptationEvent event, FrozenSystemState state) {
    	...
    	// TODO missing: the condition/measurement that triggered the adaptation, needed to derive success indicators
    }
    ...
    public void endAdaptation(boolean successful) {
    	// add the current adaptation unit to the map, using the message id of the coordination command as key
    ...
    public void enacted(CoordinationCommand command, CoordinationCommandExecutionEvent event) {
    	...
    	if(unit == null){
    		System.out.println("ERROR: adaptation unit with message id " + command.getMessageId() + " missing and could not be stored.");
    		return;
    	}
...
//    }
     ...
     * Prints a boolean.
     * 
     * @param bool the bool value to print
     */
